...
def calculate_attention(v, keys, mask, queries):
    """Attention calculation

    Based on https://arxiv.org/abs/1409.0473 

    Args:
        v: shape (dec_hidden_dim,)
    ...
    """
    h_sum = keys+queries[:, None, :] # shape: (N, L, dec_hidden_dim)
    energy = torch.sum(v[None, None, :]*torch.tanh(h_sum), 2) # shape: (N, L)
...
class DecoderModel(nn.Module):
    ...
    def forward_pass(self, input_t, keys, values, mask, ctx, input_states):
        ...
        # Calculate attention
        attn = calculate_attention(self.v, keys=new_keys, mask=mask, queries=queries)
        # Calculate context
